...
class FakeState:
    ...
    def request(self, template_id: str, context: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        objects["incident_id"] = context["incident_id"]
        ...
        response: dict[str, Any] = {
            ...
            "receipt_id": f"receipt-{self.generation}",
            "objects": objects,
        }
...
class PrepareIterationTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        generated = Path(__file__).with_name("authority-policy.projection.json")
        self.assertEqual(
            __import__("hashlib").sha256(generated.read_bytes()).hexdigest(),
    ...
    def write_projection(self, **changes: Any) -> None:
        templates = {
            ...
            for template_id, fields in prepare.EXPECTED_NORMAL_TEMPLATE_FIELDS.items()
        }
        projection = {
            "schema": 1,
            ...
            "path_classes": {
                "normal-auto": {"include": ["src/**", "meta/todos/todo.<selected-slug>.md"]},
                "resolver-auto": {"include": ["src/**"]},
            },
            "completion_tcb": {
    ...
    def test_finalisation_stops_model_before_acl_lease_and_one_snapshot(self) -> None:
        ...
        self.assertEqual(receipt["head"], "3" * 40)
