...
use firth_vm::{
    ExecutionOutcome, Image, Instruction, Op, Operand, Quotation, Value, VmError, WordEntry,
    body_digest, decode, default_registry, encode_image, evidence_digest, execute,
    execute_diagnostic, execute_diagnostic_entry, execute_diagnostic_with_stack,
...
fn quote(value: Quotation) -> Instruction {
    Instruction {
        op: Op::PushQuote,
        operand: Some(Operand::Quote(value)),
    }
}
...
fn assert_image_rejected(image: Image, expected: VmError) {
    ...
    assert_empty_trap(
        execute_diagnostic(&image, 0, &default_registry()),
        expected.clone(),
    );
    assert_empty_trap(
    ...
    #[cfg(feature = "std")]
    assert!(
        matches!(ImageStore::new(image), Err(ImageError::InvalidImage(error)) if error == expected)
    );
}
...
fn nested_code_vectors_cannot_bypass_the_limit() {
    ...
    assert_image_rejected(
        image(vec![quote(quotation.clone())]),
...
fn unused_initial_capture_indices_are_checked() {
    ...
    assert_stack_rejected(
        vec![Value::Quotation(invalid.clone())],
        VmError::InvalidCaptureIndex(0),
    );
...
fn exact_input_byte_and_vector_limits_are_accepted() {
    ...
    assert_eq!(
        execute_report_with_stack(&image, stack.clone(), 0, &default_registry())
            .unwrap()
            .stack,
        stack
    );
...
fn deepest_valid_capture_matches_the_binary_decoder() {
    ...
    let Value::Quotation(quotation) = &value else {
        panic!("quotation required")
    };
    let quoted_image = image(vec![quote(quotation.clone())]);
    assert_eq!(
        decode(&encode_image(&quoted_image)),
        Ok(quoted_image.clone())
    );
    assert_eq!(
        execute_with_fuel(&quoted_image, 0),
        Err(VmError::FuelExhausted)
    );
    let stack = vec![value];
    assert_eq!(
        execute_report_with_stack(&image(vec![]), stack.clone(), 0, &default_registry())
            .unwrap()
            .stack,
        stack
    );
...
fn valid_capture_indices_and_bitmap_at_the_vector_limit_are_accepted() {
    let quotation = Quotation {
        code: vec![Instruction {
            op: Op::PushCapture,
            operand: Some(Operand::Capture((LIMIT - 1) as u64)),
        }],
