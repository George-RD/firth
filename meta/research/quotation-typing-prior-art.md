...
checking quotation boundaries. The accepted v0.1 decision requires both branch
quotations to be `many`; the prior-art note still records that Lean must prove
the resulting preservation and at-most-once obligations. A larger
ownership-aware conditional would need to return or otherwise account for the
unselected linear capture and should not be inferred from the current
semantics.
