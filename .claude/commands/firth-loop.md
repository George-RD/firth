...
| Clean, parked detached, no unquarantined `loop/*` branches or PRs | Freshness is part of the verdict: if `git rev-parse HEAD` differs from `git rev-parse origin/main`, fast-forward the park (`git merge --ff-only origin/main`, which refuses anything but an ancestor: an ahead or diverged detached HEAD is preserved evidence for the fail-closed rows above, never discarded; clean tree, pure ref move - the supervisor refreshes only the launch checkout, never this worktree, observed 2026-08-11: selection on a week-old detached HEAD starved and re-parked a fixed incident). Then select fresh work (below). |
